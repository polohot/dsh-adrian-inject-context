 ...
 *     field default to true. enabled=true + everyTurn=true => injected
 *     before EVERY turn (legacy behavior); enabled=true + everyTurn=false
...
function makeRow(entry) {
  ...
  ftxt.textContent = 'Every turn — ticked: before every message · unticked: only the first message of a session';
      ...
      Promise.resolve().then(function () {
        ...
        return readStore().then(function (store) {
          ...
          const report = {
            ...
            header: HEADER,
            mechanism: 'agent/pre-step entering-batch prepend (dsh-agent-instructions pattern); no systemPrompt.section(), no systemPrompt.context(); never injected into an empty (turn-closing) batch; per-entry everyTurn — unticked entries inject only on a session\'s first turn and are then suppressed by in-memory per-session tracking plus persisted-surface detection',
            store: {
              schema: 2,
              mode: store.mode,
