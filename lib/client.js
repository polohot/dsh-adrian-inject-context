...
`;
		...
		function InjectContextSection() {
					...
					React.createElement("div", { className: "aic-main" },
						...
						React.createElement("div", { className: "aic-col" },
							...
							React.createElement("label", { className: "aic-freq" },
								...
								React.createElement("span", null, "Every turn \u2014 ticked: right after every message \u00b7 unticked: only the first message of a session")))),
			...
			return React.createElement("div", { className: "aic-root" },
				...
				React.createElement("p", { className: "aic-sub" },
					...
					" row injected right after your message (per entry: every turn by default, or only the first message of a session when \u201cEvery turn\u201d is unticked); the other dataset is inert. Switching tabs persists the mode immediately; edits apply from the next request."),
				rows.length === 0
		...
		// Stick-to-bottom companion. The shell's follow-the-tip rule fires only
		// when the last conversation node has kind "user"; this plugin's sourced
		// row has kind "context", so the shell stops auto-scrolling while the
		// plugin is active. While the reader sits at the bottom of the chat
		// scroller, keep the view pinned to the newest content after DOM
		// changes. Reader scrolls upward are never fought (the 25px rule
		// mirrors the shell's own at-bottom heuristic).
		function startScrollCompanion() {
			const SCROLL_MATCH = '[class*="scrollBody"]';
			const SEAT_MATCH = '[data-composer-seat]';
			const NEAR_BOTTOM_PX = 25;
			let scroller = null;
			let wasAtBottom = true;
			const atBottom = function () {
				return scroller !== null && scroller.scrollHeight - scroller.scrollTop - scroller.clientHeight <= NEAR_BOTTOM_PX;
			};
			const find = function () {
				if (scroller !== null && scroller.isConnected) return scroller;
				scroller = null;
				const candidates = document.querySelectorAll(SCROLL_MATCH);
				for (const el of candidates) {
					if (el.querySelector(SEAT_MATCH) !== null) { scroller = el; break; }
				}
				if (scroller !== null && scroller.dataset.aicPinAttached !== "1") {
					scroller.dataset.aicPinAttached = "1";
					scroller.addEventListener("scroll", function () { wasAtBottom = atBottom(); }, { passive: true });
				}
				return scroller;
			};
			const pin = function () {
				const el = find();
				if (el === null || !wasAtBottom) return;
				if (!atBottom()) el.scrollTop = el.scrollHeight;
			};
			const observer = new MutationObserver(function () { find(); pin(); });
		...
		exports.apply = function (ctx) {
			...
			let stopScrollCompanion = null;
			ctx.effect(function () {
				try {
					stopScrollCompanion = startScrollCompanion();
				} catch (error) {
					console.warn("[adrian-inject-context] scroll companion skipped: " + (error instanceof Error ? error.message : String(error)));
				}
			}, NS + ": scroll companion");
			return function () {
				if (stopScrollCompanion !== null) stopScrollCompanion();
				dropStyles();
			};
		};
