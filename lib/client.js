...
`;
		...
		function InjectContextSection() {
			...
			function addEntry() {
				replaceLessons(lessonsOf().concat([{ text: "", enabled: true }]));
			}
			...
			return React.createElement("div", { className: "aic-root" },
				...
				React.createElement("p", { className: "aic-sub" },
					...
					" row injected immediately before your message on every turn; the other dataset is inert. Switching tabs persists the mode immediately; edits apply from the next request."),
				rows.length === 0
